...
{
public:
    ProcessManagerNode() : Node("process_manager_motion_bt"), child_pid_(-1)
    {
        control_sub_ = this->create_subscription<std_msgs::msg::String>(
            "/control_process_motion_bt", 10,
            std::bind(&ProcessManagerNode::onControl, this, std::placeholders::_1));

        status_pub_ = this->create_publisher<std_msgs::msg::String>("/control_process_motion_bt_status", 10);

        timer_ = this->create_wall_timer(
            std::chrono::seconds(1),
            std::bind(&ProcessManagerNode::publishStatus, this));

        RCLCPP_INFO(this->get_logger(), 
            "ProcessManagerNode ready. Use /control_process_motion_bt with 'start' or 'stop'. Status on /bt_status");
    }
...
private:
    void onControl(const std_msgs::msg::String::SharedPtr msg)
    {
        ...
        if (cmd == "start")
        {
            ...
            RCLCPP_INFO(this->get_logger(), 
                "Starting process: ros2 run motion_plan_bt motion_plan_bt_runner");

            pid_t pid = fork();
            if (pid == 0)
            {
                setpgid(0, 0); // new process group
                execlp("ros2", "ros2", "run", "motion_plan_bt", "motion_plan_bt_runner", (char *)NULL);
                _exit(1);
            }
            else if (pid > 0)
            {
    ...
    void cleanupStrayProcesses()
    {
        ...
        while ((entry = readdir(dir)) != nullptr) {
            ...
            if (cmdline.find("motion_plan_bt_runner") != std::string::npos) {
                RCLCPP_WARN(this->get_logger(),
