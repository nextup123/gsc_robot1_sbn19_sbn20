...
class ProcessManagerRosbridgeNode : public rclcpp::Node
{
public:
    ProcessManagerRosbridgeNode() : Node("process_manager_rosbridge"), child_pid_(-1)
    {
        control_sub_ = this->create_subscription<std_msgs::msg::String>(
            "/control_process_rosbridge", 10,
            std::bind(&ProcessManagerRosbridgeNode::onControl, this, std::placeholders::_1));

        status_pub_ = this->create_publisher<std_msgs::msg::String>("/control_process_rosbridge_status", 10);

        timer_ = this->create_wall_timer(
            std::chrono::seconds(1),
            std::bind(&ProcessManagerRosbridgeNode::publishStatus, this));

        RCLCPP_INFO(this->get_logger(), 
            "ProcessManagerRosbridgeNode ready. Use /control_process_rosbridge with 'start' or 'stop'. Status on /control_process_rosbridge_status");
    }
...
private:
    void onControl(const std_msgs::msg::String::SharedPtr msg)
    {
        ...
        if (cmd == "start")
        {
            ...
            RCLCPP_INFO(this->get_logger(), 
                "Starting process: ros2 launch rosbridge_server rosbridge_websocket_launch.xml");

            pid_t pid = fork();
            if (pid == 0)
            {
                setpgid(0, 0); // new process group
                execlp("ros2", "ros2", "launch", "rosbridge_server", "rosbridge_websocket_launch.xml", (char *)NULL);
                _exit(1);
            }
            else if (pid > 0)
            {
    ...
    void cleanupStrayProcesses()
    {
        ...
        while ((entry = readdir(dir)) != nullptr) {
            ...
            if (cmdline.find("rosbridge_websocket") != std::string::npos) {
                RCLCPP_WARN(this->get_logger(),
...
int main(int argc, char **argv)
{
    ...
    rclcpp::spin(std::make_shared<ProcessManagerRosbridgeNode>());
    rclcpp::shutdown();
    return 0;
}