...
BT::NodeStatus PilzPointsPlanner::onStart()
{
    ...
    RCLCPP_INFO(node_->get_logger(),
                "[PilzPointsPlanner] Starting '%s' via %s at speed %.2f",
                current_pose_goal_.c_str(), current_planner_id_.c_str(), current_speed_factor_);

    // Start async planning based on planner type
    bool planning_started = false;
    if (current_planner_id_ == "PTP")
    {
        planning_started = startPTPPlanning(current_target_, current_speed_factor_);
        if (planning_started)
            current_state_ = State::PLANNING_PTP;
    }
    else // LIN
    {
        planning_started = startLINPlanning(current_target_, current_speed_factor_);
        if (planning_started)
...
{
    RCLCPP_WARN(node_->get_logger(), "[PilzPointsPlanner] Halted during %s",
...
// -----------------------------------------------------------------------
bool PilzPointsPlanner::startPTPPlanning(const std::vector<double>& target, double speed)
{
    // Store target and speed for use in async lambda
    auto target_copy = target;
    
    planning_future_ = std::async(std::launch::async, [this, target_copy, speed]() -> bool {
        return runPTP(target_copy, speed);
    });
...
// -----------------------------------------------------------------------
bool PilzPointsPlanner::startLINPlanning(const std::vector<double>& target, double speed)
{
    // Store target and speed for use in async lambda
    auto target_copy = target;
    
    planning_future_ = std::async(std::launch::async, [this, target_copy, speed]() -> bool {
        return runLIN(target_copy, speed);
    });
...
// -----------------------------------------------------------------------
bool PilzPointsPlanner::runPTP(const std::vector<double>& target, double speed)
{
...
// -----------------------------------------------------------------------
bool PilzPointsPlanner::runLIN(const std::vector<double>& target, double speed)
{
