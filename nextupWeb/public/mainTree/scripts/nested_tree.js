...
function buildNestedNode(nodeData, depth) {
    ...
    // 🔹 _uid is regenerated by the backend on every reload (add/move included),
    // so fall back to the stable XML ID — same reasoning as the tree-view restore
    // logic in tree_legend.js — or a freshly-added/moved node's selection drops
    // as soon as the nested view re-renders.
    const nodeId = nodeData.attributes?.ID || nodeData.id || null;
    const isSelected = !!(uid && uid === selectedNodeUid) || !!(nodeId && nodeId === selectedNodeId);
  ...
  `;
    ...
    header.addEventListener('click', () => {
        ...
        // 🔹 _uid gets regenerated by the backend on every reload, so it alone
        // won't survive an add/move refresh — also anchor to the stable XML ID
        // (mirrors the tree-view click handler in tree_legend.js) so renderTree()'s
        // restore-selection logic has a fallback to match against.
    ...
    parent: parentRaw ? {
        data: parentRaw,
        children: parentRaw.children?.map(c => ({
            data: c,
            parent: null  // shallow; enough for copyNode sibling checks
        })) ?? []
    } : null
};

        showNodeDetails(rawNode);          // showNodeDetails reads rawNode directly
        updateButtonStates(rawNode);       // updateButtonStates reads .data.type
        renderNestedView();
    });
...
function updateButtonStates(nodeData) { 
    const isWrapper = checkIsWrapper(nodeData.type);           // ✅ rawNode.type
    const uid = nodeData.attributes?._uid || nodeData.uid;     // ✅ rawNode.attributes
    const parent = findParentByUid(lastRenderedTreeData, uid); // ✅ correct UID
    ...
    const _aanBtn = document.getElementById('add-node-after-btn');
    if (_aanBtn) _aanBtn.disabled = !isNotRoot;
}
